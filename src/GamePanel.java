...
//	DataBufferByte db = (DataBufferByte)fundo.getRaster().getDataBuffer();
//	byte data[] = db.getData();
//	
//	byte dataplot[] = ((DataBufferByte)plotfundo.getRaster().getDataBuffer()).getData();
//
//	double passobase = passo/1000.0;
//	
//	for(int i = 0; i < data.length; i+=4){
//		int a = data[i]&0x00ff;
//		int b = data[i+1]&0x00ff;
...
//		int r = data[i+3]&0x00ff;
//		
//		int w = (i/4)/fundo.getWidth();
//		double noiseval = Math.sin(passobase+w/20.0);
//		double noisecos = Math.cos(passobase+w/20.0);
//
//		b = Math.min(255,(int)(b*(noiseval+1)));
//		g = Math.min(255,(int)(g*(noisecos+1)));
//		r = Math.min(255,(int)(r*(-noiseval+1)));
//		
//		dataplot[i] = (byte)(128&0x00ff);
//		dataplot[i+1] = (byte)(b&0x00ff);
//		dataplot[i+2] = (byte)(g&0x00ff);
//		dataplot[i+3] = (byte)(r&0x00ff);
//	}
}
...
//	}
	...
	for(int i = 0; i < listaDePersonagens.size();i++){
		listaDePersonagens.get(i).desenhaSe(dbg);
	}
